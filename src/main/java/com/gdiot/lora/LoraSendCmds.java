...
//		}
        if (content != "") {
            ...
            if ((LoraConfig.lora_smoke_alert).equals(deviceType)) {
                post.setHeader("userId", LoraConfig.smoke_alarm_userID);
            } else if ((LoraConfig.mqtt_smoke_alert).equals(deviceType)) {
                post.setHeader("userId", LoraConfig.smoke_alarm_userID_zs);
            } else {
            ...
            if (deveui != null && deveui != "" && deveui.length() > 0) {
                ...
                //不同用户的账号，userSec区分
                if ((LoraConfig.lora_smoke_alert).equals(deviceType)) {
                    data.put("userSec", LoraConfig.smoke_alarm_userSec);
                } else if ((LoraConfig.mqtt_smoke_alert).equals(deviceType)) {
                    data.put("userSec", LoraConfig.smoke_alarm_userSec_zs);
                } else {
