 ...
 */
@Slf4j
public class AKRSendCmdsUtils {

    public AKRSendCmdsUtils() {
...
//				logger.info("--------------getCmdsInfo---url=="+ url);
        String api_key = YDConfig.AKR_API_KEY;//api_key akr
...
//			logger.info("--------------getCmdContent start----------operate_type="+operate_type);
    ...
    public static String getAKRNBCmdMsg(String data_type, String operate_type, String value) {
        String start = "7B7B";//起始符
//		String dev_code = "AAAAAAAAAAAA";//设备编码
        String function_code = "90";//功能码：90 透传
        String modbus_data = "";//数据域：完整的modbus帧（若发送645帧，改成645帧就行）
        String CRC = "";//CRC
        String end = "7D7D";//结束符

        String content = null;
        int name_type_len = data_type.length();
        if (name_type_len == 4) {//透传  寄存器查询
    ...
    public static String getModbusCmdMsg(String name_type, String operate_type, String value) {
        String start = "01";//起始符
        String function_code = "03";//功能码：写多个寄存器
        String register_addr = "";//寄存器起始地址
        String register_count = "";//要操作的寄存器个数
        String data_byte_count = "";//  数据字节数 （占1字节）
        String data = "";//数据内容(A822寄存器)
        String CRC = "";//CRC16校验
        ...
        switch (name_type) {
//		case "":
            case "003C"://	组合有功总电能	RO	4字节 2位小数   读电量：01 03 00 3E 00 02 A5 C7
            case "003E"://	正向有功电能	RO	4字节  2位小数
                register_count = "0002";
...
//		case "01C3"://操作拉合闸控制	DO2 2字节
                if ("O".equals(operate_type)) {
                    function_code = "10";
                    register_count = "0001";
                    data_byte_count = "02";//  数据字节数 （占1字节）
                    if (value != "" && "on".equals(value)) {//合闸
                        data = "0001";
                    } else if ("off".equals(value)) {//开闸
                        data = "0000";
                    }
                ...
                break;
            case "1001"://写上传间隔：01 10 1002 0001 02 0003 B6 83
