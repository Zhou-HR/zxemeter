...
//            System.out.printf("-------para[i]="+para[i] + "\n");
        ...
        String sHex = getCS16(para);
        String crcHex = sHex;
//        System.out.printf("-------sHex="+sHex + "\n");
        if (sHex.length() == 3) {//长度为1位时，高位补0
            crcHex = "0" + sHex;
        }
        if (sHex.length() == 2) {//长度为1位时，高位补0
            crcHex = "00" + sHex;
        }
        if (sHex.length() == 1) {//长度为1位时，高位补0
            crcHex = "000" + sHex;
        ...
        //高位补0后，高低位互换
        byte[] binaryData = Utilty.hexStringToBytes(crcHex);
        String updateDataHex = Utilty.convertByteToString(binaryData, 1, binaryData.length);

//        System.out.printf("-------updateDataHex="+updateDataHex + "\n");
        return updateDataHex;
    }
     ...
     * @return
     */
    public static String getCS16(byte[] bytes) {
        int CRC = 0x0000ffff;
